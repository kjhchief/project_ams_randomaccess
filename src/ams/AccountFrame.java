...
//		String title = String.format("%9s %", accountNum);
		...
		if (Validator.isEmpty(accountNum)) {
...
//		repository = new FileAccountRepository(); // 필요한가?
		...
		saveButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					long restMoney = Long.parseLong(InputMoneyTF.getText());
					String accountInfo = null;
					if ((choice.getSelectedItem()).equals(accountChoice.MINUSACCOUNT.getAccountType())) {
						long rentMoney = Long.parseLong(rentMoneyTF.getText());
						Calendar today = Calendar.getInstance();
						accountType = "마이너스계좌";
//						

						((FileAccountRepository) repository).saveFile(
								new MinusAccount(accountNumber, accountOwner, password, restMoney, rentMoney, today));
					} else {
						accountType = "입출금계좌";
						((FileAccountRepository) repository)
								.saveFile(new Account(accountNumber, accountOwner, password, restMoney));
					}
					contentsArea.append("계좌가 정상 등록되었습니다.\n");
				} catch (Exception e1) {
					contentsArea.setText("공백 혹은 맞지않는 문자형식\n");
				}
